...
PROFILE=ROOT/'submodules/TelegramUI/Components/PeerInfo/PeerInfoScreen/Sources/PeerInfoScreenMultilineInputtem.swift'
def req(v,m):
...
def span(t,start_token,end_token,label):
    s=t.find(start_token);req(s>=0,'missing '+label+' start')
    e=t.find(end_token,s+len(start_token));req(e>s,'missing '+label+' end')
    return t[s:e]
def main():
    ...
    for token in topology_tokens:req(token in settings,'new Settings enum case is not exhaustive: '+token)
    ...
    bridge=span(settings,'private let jerkgramTelemetryEnabledKey','private enum GhostBaseSettingsEntry:','telemetry preference bridge')
    decl=span(settings,'    case aboutValue(Int32, Int32, String, String)','    case header(Int32, String)','new enum declarations')
    section=span(settings,'        case let .aboutValue(section, _, _, _):','        case let .toggle(section, _, _, _, _):','section cases')
    stable=span(settings,'        case let .aboutValue(section, index, _, _):','        case let .toggle(section, index, _, _, _):','stableId cases')
    equality=span(settings,'        case let .aboutValue(ls, li, lt, lv):','        case let .toggle(ls, li, lk, lt, lv):','equality cases')
    item=span(settings,'        case let .aboutValue(_, _, title, value):','        case let .toggle(_, _, key, title, value):','item renderer cases')
    for label,fragment in (('telemetry preference bridge',bridge),('new enum declarations',decl),('section cases',section),('stableId cases',stable),('equality cases',equality),('item renderer cases',item),('About release UI',about)):
    ...
    print('  PASS EN/RU Telegram-language strings + no Cyrillic in Build130-owned Settings fragments')
