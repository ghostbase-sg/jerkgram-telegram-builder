...
def require(value, message):
    if not value:
        raise RuntimeError("[Build122 identity] " + message)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="jerkgram-build122-identity-") as directory:
        root = Path(directory)
        ...
        require(set(plugins) == EXPECTED_EXTENSIONS, "extension topology mismatch before Build122 stamping")
        ...
        fd, temp_name = tempfile.mkstemp(prefix=ipa.name + ".build122.", suffix=".tmp", dir=str(ipa.parent))
        os.close(fd)
        temp = Path(temp_name)
        try:
    ...
    print("[Build122 identity] GREEN")
    print("[Build122 identity] stamped CFBundleVersion=122 in main app and all extensions")
