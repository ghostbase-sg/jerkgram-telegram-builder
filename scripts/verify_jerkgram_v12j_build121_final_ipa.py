...
def require(value, message):
    if not value:
        raise RuntimeError("[Build121 final IPA verify] " + message)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="jerkgram-build121-verify-") as directory:
        root = Path(directory)
        ...
        require(str(main.get("CFBundleVersion")) == EXPECTED_BUILD, "embedded main CFBundleVersion is not 121")
        ...
        for name, extension in sorted(plugins.items()):
            ...
            require(str(info.get("CFBundleVersion")) == EXPECTED_BUILD, f"{name} CFBundleVersion is not 121")
    ...
    print("[Build121 final IPA verify] GREEN")
    print("[Build121 final IPA verify] CFBundleDisplayName=Jerkgram")
    print("[Build121 final IPA verify] CFBundleVersion=121 in app + 6 extensions")
