...
def require(value, message):
    if not value:
        raise RuntimeError("[Build119 identity] " + message)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="jerkgram-build119-identity-") as directory:
        root = Path(directory)
        ...
        require(main_info.get("CFBundleDisplayName") == DISPLAY, "Build114 public display identity missing")
        require(main_info.get("CFBundleIdentifier") == PUBLIC_BUNDLE, "Build114 public bundle identity missing")

        plugins = {path.name: path for path in (app / "PlugIns").glob("*.appex") if path.is_dir()}
        require(set(plugins) == EXPECTED_EXTENSIONS, "extension topology mismatch before Build119 stamping")

        # Deliberately discover plist candidates without touching framework/resource plists.
        plist_paths = []
        ...
        for info_path in plist_paths:
            ...
            modified_members.add(info_path.relative_to(root).as_posix())

        fd, temp_name = tempfile.mkstemp(
            prefix=ipa.name + ".build119.",
            suffix=".tmp",
            dir=str(ipa.parent),
        )
        os.close(fd)
    ...
    print("[Build119 identity] GREEN")
    print("[Build119 identity] stamped CFBundleVersion=119 in main app and all extensions")
    print("[Build119 identity] Build114 public/resign-ready topology preserved")
