...
private func jerkgramDataEntries(state: JerkgramDataUIState, strings: JerkgramStrings) -> [JerkgramDataUIEntry] {
    ...
    var entries: [JerkgramDataUIEntry] = [
        .header(0, strings.retentionRules),
        .action(0, 1, strings.historyDuration, jerkgramDurationTitle(policy.historyDuration, strings: strings), "duration"),
        .action(0, 2, strings.recoveredMediaLimit, jerkgramMediaLimitTitle(policy.mediaByteLimit, strings: strings), "media"),
        .toggle(0, 3, strings.archiveSecretChats, "secretChats", policy.archiveSecretChats),
        .action(0, 4, strings.perChatRules, "", "perChat"),
        .action(0, 5, strings.cleanupExpired, "", "cleanup"),
    ]
    if policy.historyDuration == .forever && policy.mediaByteLimit == .unlimited {
        entries.append(.info(0, strings.foreverUnlimitedWarning + " (Forever + Unlimited)"))
    }
    entries.append(contentsOf: [
        .header(1, strings.backup),
        .action(1, 1, strings.exportArchive, "Build118", "export"),
        .action(1, 2, strings.importArchive, "Archive v2", "import"),
        .info(1, strings.backupAccountHint(state.configuration.accountPeerId)),
    ])
    return entries
}
