...
def require(value, message):
    if not value:
        raise RuntimeError("[Build122 final IPA verify] " + message)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="jerkgram-build122-verify-") as directory:
        root = Path(directory)
        ...
        require(str(main.get("CFBundleVersion")) == EXPECTED_BUILD, "embedded main CFBundleVersion is not 122")
        ...
        for name, extension in sorted(plugins.items()):
            ...
            require(str(info.get("CFBundleVersion")) == EXPECTED_BUILD, f"{name} CFBundleVersion is not 122")
    ...
    print("[Build122 final IPA verify] GREEN")
    print("[Build122 final IPA verify] CFBundleVersion=122 in app + 6 extensions")
