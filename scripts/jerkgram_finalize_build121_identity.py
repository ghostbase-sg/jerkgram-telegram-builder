...
def require(value, message):
    if not value:
        raise RuntimeError("[Build121 identity] " + message)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="jerkgram-build121-identity-") as directory:
        root = Path(directory)
        ...
        require(set(plugins) == EXPECTED_EXTENSIONS, "extension topology mismatch before Build121 stamping")

        plist_paths = [p for p in app.rglob("Info.plist") if p.parent == app or p.parent.suffix == ".appex"]
        ...
        fd, temp_name = tempfile.mkstemp(prefix=ipa.name + ".build121.", suffix=".tmp", dir=str(ipa.parent))
        os.close(fd)
        temp = Path(temp_name)
        try:
    ...
    print("[Build121 identity] GREEN")
    print("[Build121 identity] stamped CFBundleVersion=121 in main app and all extensions")
    print("[Build121 identity] public/resign-ready topology preserved")
