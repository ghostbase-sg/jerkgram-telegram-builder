...
def main():
    ...
    for path in ACTIVE_WORKFLOWS:
        ...
        require(workflow.count("uses: actions/upload-artifact@v4") >= 1, f"{path.name}: success artifact required")
        require(workflow.count("if: success()") == 1, f"{path.name}: exactly one success artifact required")
