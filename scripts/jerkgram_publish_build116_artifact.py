...
OUTPUT_IPA = OUTPUT_DIR / "Jerkgram-build116.ipa"
OUTPUT_INFO = OUTPUT_DIR / "Jerkgram-build116-info.txt"


def require(value, message):
    if not value:
        raise RuntimeError("[Build116 artifact] " + message)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="jerkgram-build116-artifact-") as directory:
        root = Path(directory)
    ...
    print("[Build116 artifact] GREEN")
    print("[Build116 artifact] final file:", OUTPUT_IPA)
    print("[Build116 artifact] sha256:", output_hash)
