...
OUTPUT_IPA = OUTPUT_DIR / "Jerkgram-build121.ipa"
OUTPUT_INFO = OUTPUT_DIR / "Jerkgram-build121-info.txt"

def require(value, message):
    if not value:
        raise RuntimeError("[Build121 artifact] " + message)
...
def verify_embedded_identity(source):
    with tempfile.TemporaryDirectory(prefix="jerkgram-build121-artifact-") as directory:
        root = Path(directory)
        ...
        require(str(info.get("CFBundleVersion")) == EXPECTED_BUILD, "embedded main CFBundleVersion is not 121")
        ...
        for extension in extensions:
            ...
            require(str(extension_info.get("CFBundleVersion")) == EXPECTED_BUILD, extension.name + " embedded CFBundleVersion is not 121")
...
def main():
    ...
    require("Build=121" in OUTPUT_INFO.read_text(encoding="utf-8"), "metadata build mismatch")
    print("[Build121 artifact] GREEN")
    print("[Build121 artifact] final file:", OUTPUT_IPA)
    print("[Build121 artifact] sha256:", output_hash)
