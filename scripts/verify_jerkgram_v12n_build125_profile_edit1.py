...
def main() -> None:
    for path in FILES:
        ...
        require("itemBlocksBackgroundColor.withAlphaComponent" in text, f"{path.name}: translucent theme surface missing")
        require("? 0.18 : 0.14" in text, f"{path.name}: Build125 translucency contract missing")
