...
OUTPUT_IPA = OUTPUT_DIR / "Jerkgram-build120.ipa"
OUTPUT_INFO = OUTPUT_DIR / "Jerkgram-build120-info.txt"


def require(value, message):
    if not value:
        raise RuntimeError("[Build120 artifact] " + message)
...
def verify_embedded_identity(source):
    with tempfile.TemporaryDirectory(prefix="jerkgram-build120-artifact-") as directory:
        root = Path(directory)
        ...
        require(str(info.get("CFBundleVersion")) == EXPECTED_BUILD, "embedded main CFBundleVersion is not 120")
        ...
        for extension in extensions:
            ...
            require(
                ...
                extension.name + " embedded CFBundleVersion is not 120",
            )
...
def main():
    ...
    for stale in OUTPUT_DIR.glob("Jerkgram-build11[0-9]*"):
        if stale.is_file():
            stale.unlink()
    ...
    require("Build=120" in OUTPUT_INFO.read_text(encoding="utf-8"), "metadata build mismatch")

    print("[Build120 artifact] GREEN")
    print("[Build120 artifact] final file:", OUTPUT_IPA)
    print("[Build120 artifact] sha256:", output_hash)
